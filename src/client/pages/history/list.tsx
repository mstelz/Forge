...
function sessionSubtitle(s: SessionSummary): string {
  if (s.sourceType === "routine" && s.sourceRoutineName) return s.sourceRoutineName;
  return "";
}
        ...
        <div className="flex gap-2 pb-3 overflow-x-auto no-scrollbar">
          {FILTERS.map((f) => (
            <button
              ...
              className={[
                "flex-shrink-0 rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.12em] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]",
                range === f.value
...
function SessionRow({ session }: { session: SessionSummary }) {
  ...
  return (
    <Link
      ...
      className="flex items-center gap-3 rounded-[var(--radius-card)] bg-[var(--surface)] px-3 py-3 hover:bg-[var(--surface-elevated)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]"
    >
      {/* Day number tile */}
      <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-md bg-[var(--accent)] text-lg font-bold text-[var(--accent-fg)]">
        {dayNum}
      </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
      <p className="text-sm text-[var(--text-muted)]">
        No sessions yet — finish a workout to see history here.
      </p>
    </div>
  );
}
