...
const SET_TYPE_VALUES: LogSetType[] = ["normal", "warmup", "drop", "failure", "amrap", "rest_pause"];
...
function formatSetSummary(log: SessionSetLog, weightUnit: "kg" | "lb", distanceUnit: "m" | "km" | "mi"): string {
  ...
  if (log.weightKg != null && log.reps != null) parts.push(formatStrengthSet(log, weightUnit));
    ...
    <div className="rounded-[var(--radius-card)] bg-[var(--surface)] px-4 py-4">
      ...
      {block.items.map((item, itemIdx) => {
        ...
        const extraLogs = logs
          ...
          .sort((a, b) => a.order - b.order || a.loggedAt - b.loggedAt);
            ...
            <div className="flex items-center gap-1.5">
              <h2 className="text-lg font-bold text-[var(--text)]">
                <button type="button" onClick={() => {
                  const next = item.setTargets.findIndex(slot => !logs.some(log => log.performedExerciseId === item.performedExerciseId && log.plannedSetId === slot.id && (log.status === "logged" || log.status === "skipped")));
                  if (item.setTargets.length) onSlotTap(blockIdx, itemIdx, next < 0 ? 0 : next);
                }} className="min-h-11 text-left">{prefix}{name}</button>
              </h2>
              <button
                type="button"
...
function RestTimerStrip({ timer, displaySecs, onToggle }: RestTimerStripProps) {
  ...
  return (
    <div className="relative shrink-0 overflow-hidden border-b border-[var(--border)] bg-[var(--surface)]">
      {/* progress bar */}
      <div
...
function BottomPanel({
  ...
  const {
    ...
    durationSec, durationInputStr, distanceDisplay, distanceInputStr, setType, note, segments,
  } = form;
  ...
  const extraLog = cursor?.isExtra && currentItem
    ? logs.filter(l => l.performedExerciseId === currentItem.performedExerciseId && l.status === "extra" && l.plannedSetId == null)
      .sort((a, b) => a.order - b.order || a.loggedAt - b.loggedAt)[cursor.slotIdx - currentItem.setTargets.length]
    : undefined;
  const savedExtra = extraLog && (extraLog.reps != null || extraLog.durationSec != null || extraLog.distanceM != null) ? extraLog : undefined;
  const existingLog = savedExtra ?? logs.find(l =>
    l.performedExerciseId === currentItem?.performedExerciseId && l.plannedSetId === currentSlot?.id && l.status === "logged");
  const isEditingExisting = !!existingLog;
  // A dock is mounted per selected set. Load once, independently of live-query
  // refreshes, and never let a late history result overwrite user input.
  const editedRef = useRef(false);
  const [loadingValues, setLoadingValues] = useState(true);
  useEffect(() => {
    let cancelled = false;
    const valuesFor = (log: { weightKg?: number | null; reps?: number | null; durationSec?: number | null; distanceM?: number | null }): LogFormPrefill => ({
      weightDisplay: log.weightKg != null ? Math.round(convertWeight(log.weightKg, weightUnit) * 100) / 100 : undefined,
      reps: log.reps ?? undefined,
      durationSec: log.durationSec ?? undefined,
      distanceDisplay: log.distanceM != null ? Math.round(convertDistance(log.distanceM, distanceUnit) * 1000) / 1000 : undefined,
    });
    const plannedType = currentSlot?.setType;
    const defaults: LogFormPrefill = {
      reps: currentSlot?.reps ?? currentSlot?.repsMin ?? currentItem?.uniformReps,
      setType: plannedType === "drop_set" ? "drop" : plannedType === "to_failure" ? "failure"
        : SET_TYPE_VALUES.includes(plannedType as LogSetType) ? plannedType as LogSetType : "normal",
    };
    if (existingLog) {
      dispatch({ type: "load", values: { ...valuesFor(existingLog), setType: existingLog.setType,
        rpe: existingLog.rpe ?? undefined, note: existingLog.notes ?? "",
        segments: (existingLog.segments ?? []).map(part => ({
          weight: part.weightKg != null ? String(Math.round(convertWeight(part.weightKg, weightUnit) * 100) / 100) : "",
          reps: String(part.reps), pause: String(part.pauseSec),
        })),
      } });
      setLoadingValues(false);
    } else if (currentItem) {
      dispatch({ type: "load", values: defaults });
      getLastLogValuesForExercise(currentItem.exerciseId).then(prev => {
        if (cancelled) return;
        if (!editedRef.current && prev) dispatch({ type: "load", values: { ...defaults, ...valuesFor(prev),
          segments: defaults.setType === prev.setType ? (prev.segments ?? []).map(part => ({
            weight: part.weightKg != null ? String(Math.round(convertWeight(part.weightKg, weightUnit) * 100) / 100) : "",
            reps: String(part.reps), pause: String(part.pauseSec),
          })) : [],
        } });
      }).catch(() => {
        if (!cancelled) showToast("Previous values could not be loaded. Enter this set manually.");
      }).finally(() => { if (!cancelled) setLoadingValues(false); });
    } else setLoadingValues(false);
    return () => { cancelled = true; };
    // Identity and units are encoded in the component key; live-query refreshes must not restart this load.
  ...
  const handleLogSet = async () => {
    ...
    const hasStrengthMetric = reps != null && reps > 0;
    ...
    const validateMetrics = (): string | null => {
      if (showWeightReps && !showDurationDistance && !hasStrengthMetric) return "Enter positive reps before logging.";
    ...
    const parsedSegments = showWeightReps && (setType === "drop" || setType === "rest_pause") ? segments.map(part => ({
      weightKg: part.weight.trim() ? weightToKg(Number(part.weight), weightUnit) : null,
      reps: Number(part.reps), pauseSec: setType === "rest_pause" ? Number(part.pause) : 0,
    })) : [];
    if (parsedSegments.some(part => !Number.isInteger(part.reps) || part.reps <= 0 ||
      (part.weightKg != null && (!Number.isFinite(part.weightKg) || part.weightKg < 0)) ||
      !Number.isInteger(part.pauseSec) || part.pauseSec < 0 || part.pauseSec > 3600) ||
      (parsedSegments.length > 0 && !(reps != null && reps > 0))) {
      setValidationError("Enter positive reps for every effort, a valid weight, and pause seconds from 0–3600.");
      return;
    }
    const metricFields = (now: number) => ({
      segments: parsedSegments.length ? parsedSegments : null,
      reps,
      weightKg: storedKg,
      rpe,
    ...
    if (cursor.isExtra) {
      ...
      try {
        const now = Date.now();
        const updatedExtraLog = { ...extraLog, ...metricFields(now), loggedAt: extraLog.loggedAt };
        const prevLogUpdate = savedExtra ? null : computeRestBackfill(now);
        ...
        if (savedExtra) await updateSessionLog(updatedExtraLog);
        else await updateSetBatch(updatedExtraLog, startRestTimer(now), prevLogUpdate);
    ...
    try {
      ...
      if (existingLog) {
        ...
        await updateSessionLog({ ...existingLog, ...updatedFields, loggedAt: existingLog.loggedAt });
  ...
  return (
    <div className="sticky bottom-0 flex max-h-[75dvh] flex-col border-t border-[var(--border)] bg-[var(--bg)]">
      <RestTimerStrip
        timer={timer}
      ...
      <div className="min-h-0 overflow-y-auto px-4 pb-3 pt-3 space-y-4" onChangeCapture={() => { editedRef.current = true; }} onClickCapture={() => { editedRef.current = true; }}>
        <p className="text-sm font-semibold text-[var(--text)]">{currentItem ? exerciseNames.get(currentItem.exerciseId) : ""} · Set {cursor.slotIdx + 1}</p>
        {loadingValues && <p className="text-xs text-[var(--text-muted)]">Loading previous values…</p>}
        {/* Metric inputs */}
        ...
        {showWeightReps && (setType === "drop" || setType === "rest_pause") && (
          <div className="space-y-2 rounded-xl border border-[var(--border)] p-3">
            <p className="text-xs text-[var(--text-muted)]">The main inputs are your first effort. Add each {setType === "drop" ? "drop" : "pause and effort"} below.</p>
            {segments.map((part, index) => (
              <div key={index} className="flex flex-wrap items-end gap-2">
                <span className="w-full text-xs font-semibold">{setType === "drop" ? "Drop" : "Effort"} {index + (setType === "drop" ? 1 : 2)}</span>
                {(setType === "rest_pause" ? ["pause", "weight", "reps"] as const : ["weight", "reps"] as const).map(field => (
                  <label key={field} className="min-w-0 flex-1 text-[10px] text-[var(--text-muted)]">
                    {field === "pause" ? "Pause sec" : field === "weight" ? `Weight ${weightUnit}` : "Reps"}
                    <input aria-label={`${field === "pause" ? "Pause seconds" : field === "weight" ? "Weight" : "Reps"} for ${setType === "drop" ? "drop" : "effort"} ${index + (setType === "drop" ? 1 : 2)}`}
                      type="text" inputMode={field === "weight" ? "decimal" : "numeric"} value={part[field]}
                      onFocus={e => e.target.select()}
                      onChange={e => dispatch({ type: "segmentInput", index, field, value: e.target.value })}
                      className="mt-1 h-11 w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] px-2 text-base font-bold text-[var(--text)]" />
                  </label>
                ))}
                <button type="button" aria-label={`Remove effort ${index + 2}`} onClick={() => dispatch({ type: "removeSegment", index })} className="h-11 px-2 text-[var(--text-muted)]"><TrashIcon /></button>
              </div>
            ))}
            <button type="button" disabled={segments.length >= 20} onClick={() => dispatch({ type: "addSegment" })} className="min-h-11 text-sm font-semibold text-[var(--accent)] disabled:opacity-50">+ {setType === "drop" ? "Add drop" : "Add pause + reps"}</button>
          </div>
        )}
        ...
        {!isEditingExisting && !cursor?.isExtra ? (
          <div className="flex gap-2">
            <button
              ...
              disabled={logging || loadingValues}
...
export function ActiveWorkoutPage() {
  ...
  const { weightUnit: dockWeightUnit, distanceUnit: dockDistanceUnit } = useContext(SettingsContext);
  ...
  const handleAddSet = useCallback(
    async (blockIdx: number, itemIdx: number) => {
      ...
      await qc.invalidateQueries({ queryKey: queryKeys.sessions.logs(session.id) });
      const extraSlotIdx = item.setTargets.length + exerciseLogs.filter(log => log.status === "extra" && log.plannedSetId == null).length;
      setSelectedPos({ blockIdx, itemIdx, slotIdx: extraSlotIdx, isExtra: true });
    },
    [session, liveStructure, logs, qc],
  );
  ...
  // ── Loading / no session ───────────────────────────────────────────────────
  if (sessionLoading || logsLoading || !session) {
    return <PageSkeleton />;
  }
  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      {/* Bottom panel */}
      <BottomPanel
        key={`${session.id}:${dockWeightUnit}:${dockDistanceUnit}:${activeCursor ? `${liveStructure.blocks[activeCursor.blockIdx]?.items[activeCursor.itemIdx]?.performedExerciseId}:${liveStructure.blocks[activeCursor.blockIdx]?.items[activeCursor.itemIdx]?.setTargets[activeCursor.slotIdx]?.id ?? activeCursor.slotIdx}:${!!activeCursor.isExtra}` : "done"}`}
        cursor={activeCursor}
        liveStructure={liveStructure}
        logs={logs}
        session={session}
