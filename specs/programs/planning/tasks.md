...
### 5.1 [x] Register routes + page skeleton + drawer entry
...
### 5.4 [x] OTHER PROGRAMS list (alpha sort)
...
### 5.6 [x] Delete confirmation flow
...
### 6.2 [x] Header card (name, description, durationWeeks stepper)
...
### 6.4 [x] Per-day picker sheet (reuses routine picker)
...
### 6.5 [x] Duplicate-week affordance
...
### 6.6 [x] Repeat-pattern affordance
...
### 6.8 [x] Write 3 focused builder tests
- Limit to 3:
...
### 7.1 [x] Detail page route + skeleton
...
### 7.5 [x] Day-cell long-press / overflow menu (hooks for Phase 8)
...
### 8.1 [x] "Start program" CTA on detail page
...
### 8.2 [x] Derived next-playable-day helper
...
### 8.3 [x] Skip / Unskip day actions (wiring 7.5)
...
### 8.4 [x] Auto-complete run when all non-rest days resolve
...
### 8.5 [x] End program (kebab → confirm dialog)
...
### 8.6 [x] Edit-program-during-active-run semantics
...
### 8.7 [x] Write 3 focused lifecycle tests
- Limit to 3:
